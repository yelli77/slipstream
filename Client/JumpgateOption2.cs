            ...
            try
            {
                ...
                foreach (var zone in allGates)
                {
                    ...
                    if (boards.TryGetValue(entryId, out var board))
                    {
                        ...
                        try { StarTruckMP.Log.LogInfo($"JumpgateOption2 DIAG: board='{entryId}' boardPos={board.rootObject.transform.position} gatePos={zone.transform.position} camPos={Camera.main?.transform.position} delta={Vector3.Distance(board.rootObject.transform.position, zone.transform.position):F0}m"); } catch { }
